...
Distance measures not realiable together, which seems consistent with EFA results. They represent two separate factors. They will each need to be tested individually.
...
```{r}
clean_data %>% 
  select(condition, itt_comp) %>% 
  unique() %>% 
  group_by(condition) %>% 
  summarize(mean = mean(itt_comp),
         sd = sd(itt_comp))
```

Lower scores indicate less threat and higher scores indicate higher threat.
